...
func TestExecutor_IsolationAndConstraintsWithJobChroot(t *testing.T) {
	testutil.ExecCompatible(t)

	execCmd := ExecCommand{Cmd: "/bin/ls", Args: []string{"-F", "/", "/etc/"}}
	ctx := testExecutorContext(t)
	ctx.Task.ChrootEnv = map[string]string{
		"/etc/ld.so.cache":  "/etc/ld.so.cache",
		"/etc/ld.so.conf":   "/etc/ld.so.conf",
		"/etc/ld.so.conf.d": "/etc/ld.so.conf.d",
		"/lib":              "/lib",
...
func TestExecutor_IsolationAndConstraintsWithJobChrootCompatibleToAgentChroot(t *testing.T) {
	testutil.ExecCompatible(t)
...
func TestExecutor_IsolationAndConstraintsWithJobChrootIncompatibleToAgentChroot(t *testing.T) {
	testutil.ExecCompatible(t)
